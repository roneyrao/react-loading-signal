...
describe('initial props', () => {
  ...
  test("don't show loading by setting container to false", () => {
    const spy = jest.spyOn(LoadingEvent, 'emit');
    const btn = document.createElement('button');
    const wrapper = mount(<LocalLoading button={btn} active container={false} />);
...
describe('change props', () => {
  ...
  describe('theme', () => {
    ...
    test('theme -> another theme', () => {
      ...
      const box = getBox(wrapper);
      expect(box.querySelector(`.${styles.spinning}`)).not.toBeNull();
      wrapper.setProps({ theme: Themes.Progress });
      expect(box.querySelector(`.${styles.spinning}`)).toBeNull();
      expect(box.querySelector(`.${styles.progress}`)).not.toBeNull();
    });
  });
});
